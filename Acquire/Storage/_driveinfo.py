...
def _validate_file_upload(par, file_bucket, file_key, objsize, checksum):
    ...
    file_bucket = _ObjectStore.get_bucket(bucket=bucket, bucket_name=file_bucket, create_if_needed=True)

    # check that the file uploaded matches what was promised
    (real_objsize, real_checksum) = _ObjectStore.get_size_and_checksum(file_bucket, file_key)
    ...
    if real_objsize != objsize or real_checksum != checksum:
        ...
        raise FileValidationError(
            ...
            "to upload the file again." % (real_objsize, objsize, real_checksum, checksum)
        )
...
class DriveInfo:
    ...
    def _get_metadata_bucket(self):
        ...
        try:
            return _ObjectStore.get_bucket(bucket=bucket, bucket_name=bucket_name, create_if_needed=True)
        except Exception as e:
            from Acquire.ObjectStore import RequestBucketError

            raise RequestBucketError("Unable to open the bucket '%s': %s" % (bucket_name, str(e)))
    ...
    def _resolve_acl(
        self, authorisation=None, resource=None, accept_partial_match=False, par=None, identifiers=None
    ):
        ...
        if authorisation is not None:
            ...
            if par.expired():
                raise PermissionError("Cannot access the drive at the PAR has expired!")

            if identifiers is None:
                raise PermissionError("The identifiers for the user who created the PAR " "must be passed!")

            if par.location().drive_uid() != self._drive_uid:
                raise PermissionError(
                    "Cannot access the drive as the PAR is not " "validated for this drive!"
                )
        else:
            try:
        ...
        drive_acl = self.aclrules().resolve(
            identifiers=identifiers, upstream=upstream, must_resolve=True, unresolved=False
        )
    ...
    def open_uploader(self, filename, aclrules=None, authorisation=None, par=None, identifiers=None):
        ...
        uploader = _ChunkUploader(drive_uid=self._drive_uid, file_uid=filemeta.uid())
    ...
    def close_uploader(self, file_uid, secret):
        ...
        if secret != shared_secret:
            raise PermissionError("Invalid request - you do not have permission to " "close this uploader")
        ...
        fileinfo = _FileInfo.load(drive=self, filename=filename, version=version)
    ...
    def close_downloader(self, downloader_uid, file_uid, secret):
        ...
        bucket = _get_service_account_bucket()
        key = "%s/%s/%s/%s" % (_downloader_root, self._drive_uid, file_uid, downloader_uid)
        ...
        if secret != shared_secret:
            raise PermissionError("Invalid request - you do not have permission to " "close this downloader")
    ...
    def upload_chunk(self, file_uid, chunk_index, secret, chunk, checksum):
        ...
        shared_secret = _Hash.multi_md5(shared_secret, "%s%s%d" % (self._drive_uid, file_uid, chunk_index))

        if secret != shared_secret:
            raise PermissionError(
                "Invalid chunked upload secret. You do not have permission " "to upload chunks to this file!"
            )
        ...
        if check != checksum:
            ...
            raise FileValidationError("Invalid checksum for chunk: %s versus %s" % (check, checksum))

        meta = {"filesize": len(chunk), "checksum": checksum, "compression": "bz2"}
    ...
    def download_chunk(self, file_uid, downloader_uid, chunk_index, secret):
        ...
        if data is None:
            raise PermissionError(
                "There is no downloader available to let you download " "this chunked file!"
            )
    ...
    def upload(self, filehandle, authorisation=None, encrypt_key=None, par=None, identifiers=None):
        ...
        fileinfo = _FileInfo(
            drive_uid=self._drive_uid, filehandle=filehandle, identifiers=identifiers, upstream=drive_acl
        )
        ...
        _ObjectStore.set_object(bucket=file_bucket, key=file_key, data=filedata)
        ...
        if filedata is None:
            ...
            f = _Function(
                function=_validate_file_upload,
    ...
    def download(
        ...
        fileinfo = _FileInfo.load(
            drive=self, filename=filename, version=version, identifiers=identifiers, upstream=drive_acl
        )
        ...
        if fileinfo.version().is_chunked():
            ...
            downloader = _ChunkDownloader(drive_uid=self._drive_uid, file_uid=fileinfo.version().uid())

            from Acquire.ObjectStore import ObjectStore as _ObjectStore
            from Acquire.Service import get_service_account_bucket as _get_service_account_bucket

            bucket = _get_service_account_bucket()
            key = "%s/%s/%s/%s" % (_downloader_root, self._drive_uid, filemeta.uid(), downloader.uid())
            ...
            raise PermissionError("Cannot download this file in a chunked manner!")
            ...
            ospar = _ObjectStore.create_par(
                bucket=file_bucket, encrypt_key=encrypt_key, key=file_key, readable=True, writeable=False
            )
        else:
    ...
    def is_opened_by_owner(self):
        ...
        drive_acl = self.aclrules().resolve(identifiers=identifiers, upstream=upstream)
    ...
    def list_files(
        ...
        """
        (drive_acl, identifiers) = self._resolve_acl(
            authorisation=authorisation, resource="list_files", par=par, identifiers=identifiers
        )
        ...
        if par is not None:
            if par.location().is_file():
                ...
                raise PermissionError("You do not have permission to read the Drive")

        if not drive_acl.is_readable():
            raise PermissionError("You don't have permission to read this Drive")
        ...
        if filename is not None:
            ...
            key = "%s/%s/%s" % (_fileinfo_root, self._drive_uid, _string_to_encoded(filename))
        ...
        if include_metadata:
            ...
            for name in names:
                try:
                    data = _ObjectStore.get_object_from_json(metadata_bucket, name)
                    fileinfo = _FileInfo.from_data(data, identifiers=identifiers, upstream=drive_acl)
                    filemeta = fileinfo.get_filemeta()
                    file_acl = filemeta.acl()
    ...
    def list_versions(self, filename, authorisation=None, include_metadata=False, par=None, identifiers=None):
        ...
        versions = _FileInfo.list_versions(
            drive=self,
            filename=filename,
            identifiers=identifiers,
            upstream=drive_acl,
        ...
        for version in versions:
            ...
            if aclrules is not None:
                acl = aclrules.resolve(
                    upstream=drive_acl, identifiers=identifiers, must_resolve=True, unresolved=False
                )
    ...
    def load(self, aclrules=None, autocreate=False):
        ...
        if data is None:
            if not autocreate:
                raise PermissionError("There is no drive available with UID '%s'" % self._drive_uid)
            ...
            if aclrules is not None:
                ...
                aclrule = aclrules.resolve(identifiers=self._identifiers)

                if not aclrule.is_owner():
                    raise PermissionError(
                        "You cannot set the permission of a new drive such "
                        "that you are not the owner! aclrules = %s, resolved "
                        "aclrule for identifiers = %s is %s" % (aclrules, self._identifiers, aclrule)
                    )
            ...
            data = _ObjectStore.set_ins_object_from_json(bucket, drive_key, data)
