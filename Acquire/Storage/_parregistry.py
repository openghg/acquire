...
class PARRegistry:
    ...
    def register(self, par, authorisation, secret=None):
        ...
        identifiers = authorisation.verify(
            resource="create_par %s" % par.fingerprint(), return_identifiers=True
        )
        ...
        data = {"par": par.to_data(), "identifiers": _json.dumps(identifiers), "secret": secret}
    ...
    def load(self, par_uid, secret=None):
        ...
        from Acquire.ObjectStore import validate_is_uid as _validate_is_uid
        ...
        if par.expired():
            raise PermissionError("There is no valid PAR at ID '%s' as it has expired" % par_uid)
    ...
    def resolve(self, par_uid, secret=None):
        ...
        drivemeta = _DriveMeta(name="par:%s" % par.uid(), uid=drive.uid(), aclrules=drive.aclrules())
        ...
        drive_acl = drivemeta.resolve_acl(identifiers=identifiers, open_aclrule=par.aclrule())

        if drive_acl.denied_all():
            raise PermissionError("The PAR does not have permission to access the drive")

        if par.location().is_drive():
            return drivemeta

        filemetas = drive.list_files(par=par, identifiers=identifiers, include_metadata=True)
