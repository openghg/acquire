...
class Cluster:
    ...
    def passphrase(self, resource):
        ...
        """
        if self.is_null():
            raise PermissionError("You cannot generate a passphrase from a null Cluster")
    ...
    def verify_passphrase(self, resource, passphrase):
        ...
        """
        if self.is_null():
            raise PermissionError("Invalid passphrase as this is a null Cluster")

        if passphrase != self.passphrase(resource):
            raise PermissionError("Invalid passphrase for resource %s" % resource)
    ...
    def create(service_url=None, service_uid=None, user=None):
        ...
        if Cluster._is_running_service():
            raise PermissionError("You cannot create a Cluster on a running service")
        ...
        wallet = _Wallet()
        compute_service = wallet.get_service(service_url=service_url, service_uid=service_uid)
    ...
    def get_cluster():
        ...
        if not Cluster._is_running_service():
            raise PermissionError("You can only call 'get_cluster' on the compute service")
        ...
        if data is None:
            ...
            raise ServiceError(
                "You have not set the cluster that will be used to actually " "run the compute jobs!"
            )
    ...
    def set_cluster(cluster, authorisation=None, passphrase=None, user=None):
        ...
        if Cluster._is_running_service():
            ...
            if authorisation is not None:
                ...
                cluster.verify_passphrase(passphrase=passphrase, resource="set_cluster")
            ...
            args = {"authorisation": authorisation.to_data(), "cluster": cluster.to_data()}
    ...
    def private_key(self):
        ...
        if self.is_null():
            ...
            raise PermissionError("You do not have permission to see the private key of " "this cluster")
    ...
    def rotate_keys(self):
        ...
        args = {"passphrase": passphrase, "cluster": self.to_data()}

        self.compute_service().call_function(function="set_cluster", args=args)
    ...
    def encrypt_data(self, data):
        ...
        return {
            "cluster_uid": str(self.uid()),
            "fingerprint": str(self.public_key().fingerprint()),
            "encrypted_data": _bytes_to_string(self.public_key().encrypt(_json.dumps(data))),
        }
    ...
    def decrypt_data(self, data):
        ...
        try:
            ...
            raise DecryptionError("The encrypted data is not of the correct format: %s" % str(e))
        ...
        if cluster_uid != self.uid():
            ...
            raise DecryptionError(
                ...
                "cluster - unmatched cluster UID: %s versus %s" % (cluster_uid, self.uid())
            )
    ...
    def get_pending_job_uids(self, passphrase=None):
        ...
        if Cluster._is_running_service():
            ...
            self.verify_passphrase(resource="get_pending_job_uids", passphrase=passphrase)

            bucket = _get_service_account_bucket()
            prefix = "compute/pending/"

            uids = _ObjectStore.get_all_object_names(bucket=bucket, prefix=prefix, without_prefix=True)

            return uids
        else:
            passphrase = self.passphrase(resource="get_pending_job_uids")
            args = {"passphrase": passphrase}
            result = self.compute_service().call_function(function="get_pending_job_uids", args=args)
    ...
    def to_data(self, passphrase=None):
        ...
        if passphrase is not None:
            try:
                data["private_key"] = self._private_key.to_data(passphrase=passphrase)
            except:
                pass
    ...
    def from_data(data, passphrase=None):
        ...
        if passphrase is not None:
            try:
                cluster._private_key = _PrivateKey.from_data(data["private_key"], passphrase=passphrase)
            except:
                pass
