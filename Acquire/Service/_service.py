...
def _login_service_user(service_uid):
    ...
    user = _User(username=service.service_user_name(), identity_url=service.canonical_url())
    ...
    login_args = {"short_uid": short_uid, "credentials": credentials.to_data(identity_uid=service.uid())}
...
class Service:
    ...
    def resolve(service=None, service_uid=None, service_url=None, fetch=True):
        ...
        if service is not None:
            ...
            raise TypeError(
                "You must supply one of 'service', 'service_url' or " "'service_uid' to resolve a service"
            )
        ...
        if fetch and (s["service"] is None):
            ...
            service = _get_trusted_service(
                service_uid=s["service_uid"], service_url=s["service_url"], autofetch=True
            )
    ...
    def create_stage2(self, service_uid, response):
        ...
        if not self._uid.startswith("STAGE1"):
            raise PermissionError("You cannot enter stage2 until stage1 has been completed!")

        if response != self._uid:
            raise PermissionError("Invalid response from the registry service. Not trusted!")
        ...
        (username, uid, secrets) = _create_service_user(
            service_type=self._service_type, service_uid=self._uid, service_public_key=skelkey
        )
    ...
    def __str__(self):
        if self.is_null():
            ...
            return "%s(url=%s, uid=%s)" % (self.__class__.__name__, self.canonical_url(), self.uid())
    ...
    def get_session_info(self, session_uid, scope=None, permissions=None):
        ...
        return _get_session_info(
            identity_url=self.canonical_url(), session_uid=session_uid, scope=scope, permissions=permissions
        )
    ...
    def assert_unlocked(self):
        ...
        if self.is_locked():
            raise ServiceError(
                "Cannot complete operation as the service account '%s' " "is locked" % str(self)
            )
    ...
    def should_refresh_keys(self):
        ...
        if self.is_null():
            ...
            try:
                ...
                return _get_datetime_now() > (self._last_key_update + self.key_update_interval())
            except:
                return True
    ...
    def refresh_keys(self):
        ...
        if self.is_unlocked():
            ...
            self._privcert = _PrivateKey(name="%s_refresh_privcert" % self._canonical_url)
            ...
            if self._pubkey is not None:
                ...
                args = {"challenge": encrypted_challenge, "fingerprint": self._pubkey.fingerprint()}
            ...
            if challenge is None:
                ...
                if response["response"] != challenge:
                    raise ServiceError(
                        "The requested service (%s) failed to respond " "to the challenge!" % str(service)
                    )

            if service.uid() != self.uid():
                raise ServiceError(
                    "Cannot update the service as the UID has changed. We "
                    "cannot move from %s to %s. Contact an administrator." % (str(self), str(service))
                )
    ...
    def service_user_account(self, accounting_service_url=None, accounting_service=None):
        ...
        from Acquire.Service import get_service_user_account_uid as _get_service_user_account_uid
        ...
        if not accounting_service.is_accounting_service():
            raise TypeError("The service '%s' is not an accounting service!" % str(accounting_service))

    def service_user_account_uid(self, accounting_service_url=None, accounting_service=None):
        ...
        return _get_service_user_account_uid(accounting_service_uid=accounting_service.uid())
    ...
    def call_function(self, function, args=None):
        ...
        if self.is_null():
            ...
            raise RemoteFunctionCallError("You cannot call the function '%s' on a null service!" % function)
    ...
    def verify_data(self, data):
        ...
        try:
            ...
            raise ServiceError("The signed data is not of the correct format: %s" % str(e))

        if service_uid != self.uid():
            raise ServiceError(
                "Cannot verify the data as it wasn't signed for this "
                "service - unmatched service UID: %s versus %s" % (service_uid, self.uid())
            )
    ...
    def decrypt_data(self, data):
        ...
        try:
            ...
            raise ServiceError("The encrypted data is not of the correct format: %s" % str(e))
    ...
    def load_keys(self, data):
        ...
        if self.is_unlocked():
            ...
            ranpas = self._skeleton_key.decrypt(_string_to_bytes(data["encrypted_passphrase"]))
    ...
    def get_key(self, fingerprint):
        ...
        from Acquire.Service import load_service_key_from_objstore as _load_service_key_from_objstore
        ...
        if key is None:
            ...
            raise KeyManipulationError(
                "Unable to load the key or certificate with " "fingerprint '%s'" % fingerprint
            )
        ...
        if unlocked:
            ...
            if type(key) is not _PrivateKey:
                ...
                raise KeyManipulationError(
                    "Unable to load the private key or certificate with " "fingerprint '%s'" % fingerprint
                )
        else:
            from Acquire.Crypto import PublicKey as _PublicKey
            ...
            if type(key) is not _PublicKey:
                ...
                raise KeyManipulationError(
                    "Unable to load the public key or certificate with " "fingerprint '%s'" % fingerprint
                )
    ...
    def to_data(self, password=None):
        ...
        if (self.is_unlocked()) and (password is not None):
            ...
            if self._service_user_secrets is not None:
                data["service_user_secrets"] = _bytes_to_string(self._service_user_secrets)
            else:
    ...
    def from_data(data, password=None, verify_data=False):
        ...
        if password:
            ...
            try:
                service._service_user_secrets = _string_to_bytes(data["service_user_secrets"])
            except:
                service._service_user_secrets = None

            service._skeleton_key = _PrivateKey.from_data(data["skeleton_key"], password)

            secret = service._skeleton_key.decrypt(_string_to_bytes(data["secret_data"]))
            ...
            service._privcert = _PrivateKey.from_data(secret["private_certificate"], passphrase)

            try:
                service._lastcert = _PrivateKey.from_data(secret["last_certificate"], passphrase)
            except:
                service._lastcert = service._privcert
        else:
        ...
        try:
            service._public_skeleton_key = _PublicKey.from_data(data["public_skeleton_key"])
        except:
            service._public_skeleton_key = None
