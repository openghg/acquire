...
def _get_this_service_data():
    ...
    try:
        ...
        raise ServiceAccountError("Cannot log into the service account: %s" % str(e))
...
def setup_this_service(service_type, canonical_url, registry_uid, username, password):
    ...
    if service is None:
        ...
        service = _Service.create(service_url=canonical_url, service_type=service_type)
    ...
    password = _Credentials.encode_password(password=password, identity_uid=service_uid)
    ...
    for fingerprint in oldkeys.keys():
        if fingerprint not in ["datetime", "encrypted_passphrase"]:
            _ObjectStore.set_string_object(
                bucket, "%s/oldkeys/fingerprints/%s" % (_service_key, fingerprint), key
            )
...
def get_this_service(need_private_access=False):
    ...
    try:
        ...
        raise MissingServiceAccountError(
            "Unable to read the service info from the object store! : %s" % str(e)
        )
    ...
    try:
        ...
        raise MissingServiceAccountError(
            "Unable to create the ServiceAccount object: %s %s" % (e.__class__, str(e))
        )
...
def create_service_user_account(service, accounting_service_url):
    ...
    accounting_service = service.get_trusted_service(service_url=accounting_service_url)
