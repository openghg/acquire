...
class OCIAccount:
    ...
    def _assert_valid_login_dict(login):
        ...
        needed_keys = ["user", "key_lines", "fingerprint", "tenancy", "region", "pass_phrase"]
    ...
    def create_and_connect_to_bucket(login_details, compartment, bucket_name=None):
        ...
        try:
            from oci.object_storage import ObjectStorageClient as _ObjectStorageClient
        ...
        try:
            ...
            try:
                ...
                bucket["bucket"] = client.create_bucket(client.get_namespace().data, request).data
                ...
                try:
                    ...
                    raise AccountError(
                        "Cannot access the bucket '%s' : %s (originally %s)" % (bucket_name, str(e), str(e1))
                    )
        except Exception:
