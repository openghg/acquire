...
class RemoteFunctionCallError(Exception):
    ...
    def __init__(self, message, child_exception=None):
        ...
        if child_exception is not None:
            if issubclass(child_exception.__class__, Exception):
                from ._function import exception_to_safe_exception as _exception_to_safe_exception

                self._child_exception = _exception_to_safe_exception(child_exception)
            else:
    ...
    def unpack_and_raise(self):
        if self._child_exception is None:
            ...
            tb_lines = _traceback.format_exception(self.__class__, self, self.__traceback__)

            self._child_exception.args = (
                "%s\n=== Local Traceback ===\n%s" % (self._child_exception.args[0], "".join(tb_lines)),
            )
