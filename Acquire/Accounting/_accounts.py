...
class Accounts:
    ...
    def _root(self):
        ...
        return "accounting/account_groups/%s" % _string_to_encoded(self._group)
    ...
    def _get_aclrules(self, user_guid, aclrules, bucket=None):
        ...
        try:
            self._aclrules = _ACLRules.from_data(_ObjectStore.get_object_from_json(bucket=bucket, key=aclkey))
        except:
            self._aclrules = None
        ...
        if aclrules is None:
            if user_guid is None:
                raise PermissionError(
                    "You must specify the guid of the initial user who " "owns this account!"
                )
        ...
        _ObjectStore.set_object_from_json(bucket=bucket, key=aclkey, data=aclrules.to_data())
    ...
    def _acls_key(self):
        ...
        return "accounting/account_group_acls/%s" % _string_to_encoded(self._group)
    ...
    def _assert_is_readable(self):
        ...
        aclrule = self._aclrules.resolve(must_resolve=True, identifiers=self.identifiers())

        if not aclrule.is_readable():
            raise PermissionError("You do not have permission to access the accounts " "in this group")
    ...
    def _assert_is_writeable(self):
        ...
        if not aclrule.is_writeable():
            raise PermissionError("You do not have permission to write to the accounts " "in this group")
    ...
    def get_account(self, name, bucket=None):
        ...
        try:
            ...
            account_uid = _ObjectStore.get_string_object(bucket, self._account_key(name))
        except:
            account_uid = None
        ...
        if account_uid is None:
            ...
            if name == "main":
                return self.create_account("main", "primary user account", overdraft_limit=0, bucket=bucket)

            from Acquire.Accounting import AccountError

            raise AccountError("There is no account called '%s' in the " "group '%s'" % (name, self.group()))
    ...
    def contains(self, account, bucket=None):
        ...
        try:
            ...
            account_uid = _ObjectStore.get_string_object(bucket, self._account_key(account.name()))
        except:
            account_uid = None

        return account.uid() == account_uid

    def create_account(self, name, description=None, overdraft_limit=None, bucket=None, authorisation=None):
        ...
        try:
            _ObjectStore.set_string_object(bucket, account_key, "under_construction")
        except:
            m.unlock()
            raise
