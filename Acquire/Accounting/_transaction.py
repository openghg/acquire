...
def _create_decimal(value):
    ...
    if d < 0:
        ...
        raise TransactionError("You cannot create a transaction with a negative value (%s)" % (value))

    elif d >= 1000000000000000:
        from Acquire.Accounting import TransactionError

        raise TransactionError(
            "You cannot create a transaction with a value greater than " "1 quadrillion! (%s)" % (value)
        )
...
class Transaction:
    ...
    def __init__(self, value=0, description=None):
        ...
        if self._value < 0:
            ...
            raise TransactionError(
                "You cannot create a transaction (%s) with a " "negative value! %s" % (description, value)
            )
        ...
        if description is None:
            if self._value > 0:
                ...
                raise TransactionError(
                    "You must give a description to all non-zero " "transactions! %s" % self.value()
                )
            else:
    ...
    def split(value, description):
        ...
        if value < Transaction.maximum_transaction_value():
            ...
            for i in range(0, len(values)):
                transactions.append(
                    Transaction(values[i], "%s: %d of %d" % (description, i + 1, len(values)))
                )
            ...
            if total != orig_value:
                ...
                raise TransactionError(
                    "Error as split sum (%s) is not equal to the original " "value (%s)" % (total, orig_value)
                )
