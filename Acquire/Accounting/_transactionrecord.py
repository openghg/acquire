...
class TransactionRecord:
    ...
    def __eq__(self, other):
        if isinstance(other, self.__class__):
            return (
                self._debit_note == other._debit_note
                and self._credit_note == other._credit_note
    ...
    def assert_matching_refund(self, refund):
        ...
        if self.is_null():
            if not refund.is_null():
                raise UnmatchedRefundError("%s does not match a null TransactionRecord!" % str(refund))
        ...
        if refund.transaction_uid() != self.uid():
            match = False
            errors.append("UID: %s != %s" % (refund.transaction_uid(), self.uid()))
        elif refund.debit_account_uid() != self.debit_account_uid():
            match = False
            errors.append("DEBIT: %s != %s" % (refund.debit_account_uid(), self.debit_account_uid()))
        elif refund.credit_account_uid() != self.credit_account_uid():
            match = False
            errors.append("CREDIT: %s != %s" % (refund.credit_account_uid(), self.credit_account_uid()))
    ...
    def assert_matching_receipt(self, receipt):
        ...
        if self.is_null():
            if not receipt.is_null():
                raise UnmatchedReceiptError("%s does not match a null TransactionRecord!" % str(receipt))
        ...
        if receipt.transaction_uid() != self.uid():
            match = False
            errors.append("UID: %s != %s" % (receipt.transaction_uid(), self.uid()))
        elif receipt.debit_account_uid() != self.debit_account_uid():
            match = False
            errors.append("DEBIT: %s != %s" % (receipt.debit_account_uid(), self.debit_account_uid()))
        elif receipt.credit_account_uid() != self.credit_account_uid():
            match = False
            errors.append("CREDIT: %s != %s" % (receipt.credit_account_uid(), self.credit_account_uid()))
    ...
    def _load_transaction(self, uid, bucket=None):
        ...
        self.__dict__ = _copy(_Ledger.load_transaction(uid, bucket=bucket).__dict__)
    ...
    @staticmethod
    def load_test_and_set(uid, expected_state, new_state, bucket=None):
        ...
        try:
            ...
            raise LedgerError(
                "Cannot secure a Ledger mutex for transaction " "'%s'. Error = %s" % (uid, str(e))
            )
        ...
        if mutex.seconds_remaining_on_lease() < 100:
            ...
            return TransactionRecord.load_test_and_set(uid, expected_state, new_state, bucket)
    ...
    def from_data(data):
        ...
        if data and len(data) > 0:
            ...
            record._transaction_state = TransactionState(data["transaction_state"])
