...
class CreditNote:
    ...
    def _create_from_refund(self, debit_note, refund, account, bucket):
        ...
        transaction = _TransactionRecord.load_test_and_set(
            refund.transaction_uid(), _TransactionState.REFUNDING, _TransactionState.REFUNDING, bucket=bucket
        )
        ...
        if account is None:
            ...
            raise ValueError(
                "The accounts do not match when refunding "
                "the receipt: %s versus %s" % (account.uid(), refund.debit_account_uid())
            )
        ...
        _TransactionRecord.load_test_and_set(
            refund.transaction_uid(), _TransactionState.REFUNDING, _TransactionState.REFUNDED, bucket=bucket
        )
    ...
    def _create_from_receipt(self, debit_note, receipt, account, bucket):
        ...
        if account is None:
            ...
            raise ValueError(
                "The accounts do not match when crediting "
                "the receipt: %s versus %s" % (account.uid(), receipt.credit_account_uid())
            )
