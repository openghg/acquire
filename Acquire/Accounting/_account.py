...
def _get_last_month(datetime):
    ...
    return _datetime.datetime(year=datetime.year, month=datetime.month, day=1, tzinfo=datetime.tzinfo)
...
def _get_hourly_datetime(datetime):
    ...
    return _datetime.datetime(
        year=datetime.year, month=datetime.month, day=datetime.day, hour=datetime.hour, tzinfo=datetime.tzinfo
    )
...
def _get_key_from_day(start, datetime):
    ...
    datetime = _datetime_to_datetime(datetime)
    return "%s/%4d-%02d-%02d" % (start, datetime.year, datetime.month, datetime.day)
...
def _get_hour_from_key(key):
    ...
    if m:
        from Acquire.ObjectStore import date_and_time_to_datetime as _date_and_time_to_datetime
        import datetime as _datetime

        return _date_and_time_to_datetime(
            _datetime.date(year=int(m.groups()[0]), month=int(m.groups()[1]), day=int(m.groups()[2])),
...
def _get_datetime_from_key(key):
    ...
    m = _re.search(r"(\d\d\d\d)-(\d\d)-(\d\d)T(\d\d):(\d\d):(\d\d)\.(\d+)", key)

    if m:
        from Acquire.ObjectStore import date_and_time_to_datetime as _datetime_to_datetime
        import datetime as _datetime
...
class Account:
    ...
    def __init__(self, name=None, description=None, uid=None, aclrules=None, group_name=None, bucket=None):
        ...
        if uid is not None:
            ...
            if name:
                if name != self.name():
                    ...
                    raise AccountError(
                        "This account name '%s' does not match what you " "expect! '%s'" % (self.name(), name)
                    )
            ...
            self._create_account(
                name=name, description=description, group_name=group_name, aclrules=aclrules, bucket=bucket
            )
    ...
    def __str__(self):
        if self._uid is None:
            ...
            return "Account(%s|%s|%s)" % (self._name, self._description, self._uid)
    ...
    def _create_account(self, name, description, group_name, aclrules, bucket=None):
        """Create the account from scratch"""
        if name is None or description is None:
            from Acquire.Accounting import AccountError

            raise AccountError("You must pass both a name and description to create a new " "account")
    ...
    def _get_transactions_between(self, start_datetime, end_datetime, bucket=None):
    ...
    def _get_balance_key(self, now=None):
        ...
        if self.is_null():
            ...
            return _get_key_from_hour(start=self._balance_key(), datetime=self._get_now(now))
    ...
    def _find_last_balance_key(self, now=None, bucket=None):
        ...
        _ObjectStore.set_object_from_json(bucket=bucket, key=hourly_key, data=hourly_balance.to_data())
    ...
    def _get_hourly_balance(self, now=None, bucket=None):
        ...
        try:
            data = _ObjectStore.get_object_from_json(bucket=bucket, key=hourly_key)
        ...
        if hourly_balance is None:
            ...
            last_balance_key = self._find_last_balance_key(now=now, bucket=bucket)

            if last_balance_key is None:
                from Acquire.Accounting import AccountError

                raise AccountError("The first balance of the account %s has not been set?" % str(self))

            data = _ObjectStore.get_object_from_json(bucket=bucket, key=last_balance_key)
            ...
            transactions = self._get_transactions_between(
                start_datetime=last_balance_time, end_datetime=hourly_now_time
            )
    ...
    def balance(self, now=None, bucket=None):
        ...
        if hourly_update is None:
            hourly_balance = self._get_hourly_balance(bucket=bucket, now=now)
        ...
        transactions = self._get_transactions_between(
            start_datetime=last_update_time, end_datetime=now, bucket=bucket
        )
    ...
    def assert_valid_authorisation(self, authorisation, resource=None, accept_partial_match=False):
        ...
        if not isinstance(authorisation, _Authorisation):
            raise TypeError("The passed authorisation must be an " "Authorisation")
        ...
        if not authorisation.is_null():
            identifiers = authorisation.verify(
                resource=resource, accept_partial_match=accept_partial_match, return_identifiers=True
            )
        ...
        if self.group_name() is not None:
            ...
            upstream = group.aclrules().resolve(must_resolve=False, identifiers=identifiers)

        aclrule = self._aclrules.resolve(must_resolve=True, upstream=upstream, identifiers=identifiers)

        if not aclrule.is_writeable():
            raise PermissionError("You do not have permission to write (draw funds) from " "this account!")
    ...
    def _credit_refund(self, debit_note, refund, bucket=None):
        ...
        if refund.value() != debit_note.value():
            raise ValueError(
                "The refunded value does not match the value "
                "of the debit note: %s versus %s" % (refund.value(), debit_note.value())
            )

        from Acquire.Accounting import TransactionInfo as _TransactionInfo
        from Acquire.Accounting import TransactionCode as _TransactionCode

        encoded_value = _TransactionInfo.encode(_TransactionCode.RECEIVED_REFUND, refund.value())
    ...
    def _debit_refund(self, refund, bucket=None):
        ...
        encoded_value = _TransactionInfo.encode(_TransactionCode.SENT_REFUND, refund.value())
    ...
    def _credit_receipt(self, debit_note, receipt, bucket=None):
        ...
        if receipt.receipted_value() != debit_note.value():
            raise ValueError(
                "The receipted value does not match the value "
                "of the debit note: %s versus %s" % (receipt.receipted_value(), debit_note.value())
            )
        ...
        encoded_value = _TransactionInfo.encode(
            _TransactionCode.SENT_RECEIPT, receipt.value(), receipt.receipted_value()
        )
    ...
    def _debit_receipt(self, receipt, bucket=None):
        ...
        encoded_value = _TransactionInfo.encode(
            _TransactionCode.RECEIVED_RECEIPT, receipt.value(), receipt.receipted_value()
        )
    ...
    def _credit(self, debit_note, bucket=None):
        ...
        if debit_note.is_provisional():
            encoded_value = _TransactionInfo.encode(_TransactionCode.ACCOUNT_RECEIVABLE, debit_note.value())
        else:
            encoded_value = _TransactionInfo.encode(_TransactionCode.CREDIT, debit_note.value())
    ...
    def _debit(
        self, transaction, authorisation, is_provisional, receipt_by, authorisation_resource=None, bucket=None
    ):
        ...
        if balance.available(self.get_overdraft_limit()) < transaction.value():
            ...
            raise InsufficientFundsError(
                ...
                "are insufficient funds in this account." % (transaction, str(self))
            )
        ...
        while True:
            ...
            if is_provisional:
                ...
                if delta < 3600:
                    ...
                    raise AccountError(
                        ...
                        % (_datetime_to_string(receipt_by), _datetime_to_string(now), delta)
                    )
            else:
                receipt_by = None
            ...
            if is_provisional:
                ...
                encoded_value = _TransactionInfo.encode(_TransactionCode.DEBIT, transaction.value())
        ...
        _ObjectStore.set_object_from_json(bucket=bucket, key=item_key, data=line_item.to_data())
        ...
        if balance.available(overdraft_limit=self._overdraft_limit) < 0:
            ...
            item_key = "%s/%s" % (self._transactions_key(), info.to_key())
    ...
    def set_overdraft_limit(self, limit, bucket=None):
        ...
        if limit < 0:
            raise ValueError("You cannot set the overdraft limit to a " "negative value! (%s)" % limit)
        ...
        if old_limit != limit:
            ...
            if self.is_beyond_overdraft_limit():
                ...
                raise AccountError(
                    "You cannot change the overdraft limit to "
    ...
    def _load_account(self, bucket=None):
        ...
        try:
            data = _ObjectStore.get_object_from_json(bucket=bucket, key=self._key())
        except:
            data = None
        ...
        if data is None:
            ...
            raise AccountError("There is no account data for this account? %s" % self._key())
    ...
    def _save_account(self, bucket=None):
        ...
        bucket = self._get_account_bucket()
        _ObjectStore.set_object_from_json(bucket=bucket, key=self._key(), data=self.to_data())
