...
canonical_url = get_service_url(service="registry")
...
_registries = {
    _testing_registry["uid"]: _testing_registry,
...
def get_registry_details(registry_uid):
    ...
    if registry["public_key"] is None:
        try:
            ...
            _keys = _import_module(
                "._keys_%s" % registry_uid, package="Acquire.Registry"
            )
