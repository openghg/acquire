...
def register_service(service, registry_uid):
    ...
    if not service.uid().startswith("STAGE1"):
        raise PermissionError("You can only register services that are at STAGE1 of " "construction")
    ...
    if service.service_type() == "registry":
        ...
        if canonical_url != service.canonical_url():
            raise PermissionError(
                ...
                "you are trying to set to %s" % (service.canonical_url(), details["canonical_url"])
            )

        from Acquire.Registry import update_registry_keys_and_certs as _update_registry_keys_and_certs
    ...
    result = registry_service.call_function(function="register_service", args=args)
