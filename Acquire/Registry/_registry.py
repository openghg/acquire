...
class Registry:
    ...
    def challenge_service(self, service):
        ...
        args = {
            "challenge": _bytes_to_string(encrypted_challenge),
    ...
    def get_service(self, service_uid=None, service_url=None):
        ...
        if service_url is not None:
            ...
            service_url2 = get_service_url(service_url)

            assert service_url == service_url2
        ...
        if service is not None:
            ...
            if service.uid() == "STAGE1":
                ...
                try:
                    _ObjectStore.set_string_object(
                        bucket=bucket, key=active_key, string_data=uidkey
                    )
                except:
                    pass
            ...
            if must_write:
                ...
                _ObjectStore.set_object_from_json(
                    bucket=bucket, key=service_key, data=data
                )
            return service
    ...
    def register_service(self, service, force_new_uid=False):
        ...
        try:
            pending_keys = _ObjectStore.get_all_object_names(
                bucket=bucket, prefix="%s/pending/" % domainroot
            )
        ...
        if service_uid is None:
            ...
            service_uid = _generate_service_uid(
                bucket=self.get_bucket(), registry_uid=self.registry_uid()
            )
        ...
        _ObjectStore.set_object_from_json(
            bucket=bucket, key=uidkey, data=service.to_data()
        )
