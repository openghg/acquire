...
def _generate_service_uid(bucket, registry_uid):
    ...
    try:
        last_vals = _ObjectStore.get_object_from_json(bucket=bucket, key=key)
...
class Registry:
    ...
    def __init__(self):
        ...
        self._registry_uid = _get_this_service(need_private_access=False).uid()
    ...
    def _get_domain(self, url):
        ...
        if domain is None or len(domain) == 0:
            if self.is_test_registry():
                ...
                raise ValueError("Cannot extract the domain from '%s'" % url)
    ...
    def get_service_key(self, service_uid=None, service_url=None):
        ...
        if service_uid is not None:
            ...
            try:
                ...
                service_key = _ObjectStore.get_string_object(bucket=bucket, key=key)
            except:
                service_key = None
    ...
    def challenge_service(self, service):
        ...
        args = {"challenge": _bytes_to_string(encrypted_challenge), "fingerprint": pubkey.fingerprint()}
        ...
        if result["response"] != challenge:
            raise PermissionError(
                "Failure of the service %s to correctly respond " "to the challenge!" % service
            )
    ...
    def get_service(self, service_uid=None, service_url=None):
        ...
        service_key = self.get_service_key(service_uid=service_uid, service_url=service_url)

        service = None

        if service_key is not None:
            try:
                data = _ObjectStore.get_object_from_json(bucket=bucket, key=service_key)
        ...
        if service is not None:
            ...
            if service.uid() == "STAGE1":
                ...
                try:
                    _ObjectStore.delete_object(bucket=bucket, key=pending_key)
                except:
                    pass

                try:
                    _ObjectStore.set_string_object(bucket=bucket, key=active_key, string_data=uidkey)
                except:
                    pass
            ...
            if must_write:
                ...
                _ObjectStore.set_object_from_json(bucket=bucket, key=service_key, data=data)
            return service
        ...
        raise MissingServiceError(
            "No service available: service_url=%s  service_uid=%s" % (service_url, service_uid)
        )
    ...
    def register_service(self, service, force_new_uid=False):
        ...
        try:
            pending_keys = _ObjectStore.get_all_object_names(bucket=bucket, prefix="%s/pending/" % domainroot)
        ...
        try:
            active_keys = _ObjectStore.get_all_object_names(bucket=bucket, prefix="%s/active/" % domainroot)
        ...
        try:
            uidkey = _ObjectStore.get_string_object(bucket=bucket, key=urlkey)
        except:
            uidkey = None
        ...
        if service_uid is None:
            ...
            service_uid = _generate_service_uid(bucket=self.get_bucket(), registry_uid=self.registry_uid())

        # save this service to the object store
        uidkey = self._get_key_for_uid(service_uid)

        _ObjectStore.set_object_from_json(bucket=bucket, key=uidkey, data=service.to_data())

        _ObjectStore.set_string_object(bucket=bucket, key=urlkey, string_data=uidkey)

        domainkey = self._get_root_key_for_domain(domain=domain)

        _ObjectStore.set_string_object(
            bucket=bucket, key="%s/pending/%s" % (domainkey, service_uid), string_data=uidkey
        )
