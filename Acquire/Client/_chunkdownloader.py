...
class ChunkDownloader:
    ...
    def _start_download(self, filename=None, directory=None):
        ...
        if self.is_null():
            raise PermissionError("Cannot download a chunk using a null uploader!")

        if filename is None:
            filename = self._last_filename

        if filename is None:
            raise PermissionError("You must supply a filename to which to download the file!")
        ...
        if self._last_filename is None:
            ...
            self._downloaded_filename = _create_new_file(filename=filename, directory=directory)
            self._FILE = open(self._downloaded_filename, "ab")
        elif self._last_filename != filename:
            raise PermissionError("You cannot change the filename during an active " "streaming download!")
    ...
    def download_next_chunk(self):
        ...
        if service is None:
            raise PermissionError("Cannot download a chunk from a null service!")

        from Acquire.Crypto import Hash as _Hash

        secret = _Hash.multi_md5(self._secret, "%s%s%d" % (self._drive_uid, self._file_uid, self._next_index))
        ...
        if "meta" in response:
            ...
            if checksum != md5:
                ...
                raise FileValidationError(
                    "Problem downloading - checksums don't agree: %s vs %s" % (checksum, md5)
                )
    ...
    def close(self):
        ...
        if self.is_open():
            ...
            self.service().call_function(function="close_downloader", args=args)
