...
class Credentials:
    ...
    def __init__(self, username=None, short_uid=None, device_uid=None, password=None, otpcode=None):
    ...
    def from_data(data, username, short_uid, random_sleep=150):
        ...
        """
        result = Credentials.unpackage(
            data=data, username=username, short_uid=short_uid, random_sleep=random_sleep
        )
    ...
    def assert_matching_username(self, username):
        ...
        if self.is_null() or self._username != username:
            raise PermissionError("Disagreement for the username for the matched " "credentials")
    ...
    def encode_password(password, identity_uid, device_uid=None):
        ...
        encoded_password = Credentials.encode_device_uid(
            encoded_password=encoded_password, device_uid=device_uid
        )

        return encoded_password

    @staticmethod
    def package(identity_uid, short_uid, username, password, otpcode, device_uid=None):
        ...
        if username is None or password is None or otpcode is None:
            raise PermissionError("You must supply a username, password and otpcode " "to be able to log in!")

        encoded_password = Credentials.encode_password(
            identity_uid=identity_uid, device_uid=device_uid, password=password
        )
