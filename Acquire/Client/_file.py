...
class File:
    ...
    def open(metadata, creds=None):
        ...
        if creds.is_par():
            ...
            if not loc.is_drive():
                ...
                d = _DriveMeta(name="par:%s" % par.uid(), uid=loc.drive_uid())
    ...
    def chunk_upload(self, aclrules=None):
        ...
        args = {"filename": uploaded_name, "drive_uid": drive_uid}
        ...
        if self._creds.is_user():
            ...
            raise PermissionError("Either a logged-in user or valid PAR must be provided!")
        ...
        response = storage_service.call_function(function="open_uploader", args=args)

        filemeta = _FileMeta.from_data(response["filemeta"])

        filemeta._set_drive_metadata(self._metadata._drive_metadata, self._creds)

        self._metadata = filemeta

        from Acquire.Client import ChunkUploader as _ChunkUploader

        return _ChunkUploader.from_data(response["uploader"], privkey=privkey, service=storage_service)
    ...
    def upload(self, filename, force_par=False, aclrules=None):
        ...
        try:
            ...
            if not filehandle.is_localdata():
                ...
                par.write(privkey).set_object_from_file(filehandle.local_filename())
                par.close(privkey)
    ...
    def chunk_download(self, filename=None, version=None, dir=None):
        ...
        if self._creds.is_user():
            ...
            authorisation = _Authorisation(
                resource="download %s %s" % (drive_uid, self._metadata.name()), user=self._creds.user()
            )
            args["authorisation"] = authorisation.to_data()
        elif self._creds.is_par():
            par = self._creds.par()
    ...
    def list_versions(self, include_metadata=False):
        ...
        args = {"drive_uid": drive_uid, "include_metadata": include_metadata, "filename": filename}

        if self._creds.is_user():
            from Acquire.Client import Authorisation as _Authorisation

            authorisation = _Authorisation(resource="list_versions %s" % filename, user=self._creds.user())
            args["authorisation"] = authorisation.to_data()
        elif self._creds.is_par():
            par = self._creds.par()
            par.assert_valid()
        ...
        response = storage_service.call_function(function="list_versions", args=args)
