...
def _get_storage_service(storage_url=None):
    ...
    if not service.is_storage_service():
        ...
        raise LoginError(
            ...
            "The service at '%s' is a '%s'" % (storage_url, service.service_type())
        )
...
class StorageCreds:
    ...
    def __init__(
        self, user=None, service=None, storage_service=None, service_url=None, par=None, secret=None
    ):
        ...
        if user is not None:
            ...
            if service is not None:
                ...
                storage_service = _Service.resolve(service, fetch=True)["service"]
            ...
            if par.expired():
                raise PermissionError("The passed PAR is either invalid or expired!")
    ...
    def user(self):
        if self._user is None:
            raise PermissionError("There is no user associated with these credentials!")
        return self._user

    def par(self):
        if self._par is None:
            raise PermissionError("There is no PAR associated with these credentials!")
    ...
    def storage_service(self):
        if self._storage_service is None:
            raise PermissionError("There is no storage service associated with these " "credentials!")
