...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Log into an Acquire-based identity " "service via a login url", prog="acquire_login"
    )

    parser.add_argument("url", type=str, nargs="*", help="Login URL")

    parser.add_argument("-u", "--username", type=str, nargs="?", help="Username with which to log in")

    parser.add_argument(
        "--remember-password", action="store_true", default=True, help="Remember the password (default on)"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=None,
        help="Do a dry-run of the login - don't connect to " "the server",
    )
