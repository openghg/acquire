...
class Job:
    ...
    @staticmethod
    def submit(input, resources, account, user=None, max_spend=None, max_runtime=None, access_service=None):
        ...
        if max_spend is None or max_runtime is None:
            ...
            result = access_service.call_function(function="instrument_resources", args=args)

            request = _RunRequest.from_data(result["request"])

        job._request = request

        cheque = account.write_cheque(
            recipient=access_service,
