...
__all__ = ["Account", "get_accounts", "create_account", "deposit", "withdraw"]
...
def _get_accounting_service(accounting_url=None):
    ...
    if not service.is_accounting_service():
        ...
        raise LoginError(
            ...
            "The service at '%s' is a '%s'" % (accounting_url, service.service_type())
        )

    return service


def _get_account_uid(user, account_name, accounting_service=None, accounting_url=None):
    ...
    if accounting_service is None:
        ...
        if not accounting_service.is_accounting_service():
            raise TypeError("You can only query accounts using " "a valid accounting service")
    ...
    raise AccountError("There is no account called '%s' for '%s'" % (account_name, str(user)))
...
def get_accounts(user, accounting_service=None, accounting_url=None):
    ...
    if accounting_service is None:
        ...
        if not accounting_service.is_accounting_service():
            raise TypeError("You can only query account using " "a valid accounting service")
        service = accounting_service

    account_uids = _get_account_uids(user, accounting_service=service)
...
def create_account(user, account_name, description=None, accounting_service=None, accounting_url=None):
    ...
    if not user.is_logged_in():
        raise PermissionError(
            ...
            "'%s' as the user login has not been authenticated." % (account_name, user.name())
        )

    from Acquire.Client import Authorisation as _Authorisation

    authorisation = _Authorisation(user=user, resource="create_account %s" % account_name)

    args = {"account_name": str(account_name), "authorisation": authorisation.to_data()}

    if description is None:
        args["description"] = "Account '%s' for '%s'" % (str(account_name), user.name())
    else:
...
def deposit(user, value, description=None, account_name=None, accounting_service=None, accounting_url=None):
    ...
    if accounting_service is None:
        ...
        if not accounting_service.is_accounting_service():
            raise TypeError("You can only deposit funds using an " "accounting service!")
        service = accounting_service
    ...
    authorisation = _Authorisation(user=user, resource=transaction.fingerprint())

    args = {"authorisation": authorisation.to_data(), "transaction": transaction.to_data()}
...
def withdraw(user, value, description=None, accounting_service=None, accounting_url=None):
...
class Account:
    ...
    def __init__(
        ...
        if user is not None:
            ...
            if not isinstance(accounting_service, _AccountingService):
                raise TypeError("You can only create an account on a valid " "AccountingService object")

            if not accounting_service.is_accounting_service():
                raise TypeError(
                    "The passed service - %s - is not an accounting " "service." % str(accounting_service)
                )
            ...
            if self._account_uid is None:
                ...
                self._account_uid = _get_account_uid(user, self._account_name, accounting_service)
        else:
            self._account_uid = None
    ...
    def deposit(self, value, description=None):
        ...
        """
        if not self.is_logged_in():
            raise PermissionError("You must log into this account to deposit new funds")
    ...
    def perform(self, transaction, credit_account, is_provisional=False):
        ...
        if not self.is_logged_in():
            raise PermissionError(
                "You cannot transfer value from '%s' to "
                "'%s' because you have not authenticated "
                "the user who owns this account" % (str(self), str(credit_account))
            )

        from Acquire.Accounting import Transaction as _Transaction

        if not isinstance(transaction, _Transaction):
            raise TypeError("The passed transaction must be of type " "Transaction")

        if not isinstance(credit_account, Account):
            raise TypeError("The passed credit account must be of type " "Account")
        ...
        auth = _Authorisation(resource=transaction.fingerprint(), user=self._user)
    ...
    def receipt(self, credit_note, receipted_value=None):
        ...
        """
        if not self.is_logged_in():
            raise PermissionError("You cannot receipt a credit note as the " "user has not yet logged in!")

        if credit_note.account_uid() != self.uid():
            raise ValueError(
                "You cannot receipt a transaction from a different "
                "account! %s versus %s" % (credit_note.account_uid(), self.uid())
            )
        ...
        args = {"credit_note": credit_note.to_data(), "authorisation": auth.to_data()}
    ...
    def refund(self, credit_note):
        ...
        """
        if not self.is_logged_in():
            raise PermissionError("You cannot refund a credit note as the " "user has not yet logged in!")
    ...
    def write_cheque(self, recipient, resource, max_spend, expiry_date=None):
        ...
        return _Cheque.write(
            account=self, resource=resource, recipient=recipient, max_spend=max_spend, expiry_date=expiry_date
        )
