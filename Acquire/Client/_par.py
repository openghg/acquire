class PAR:
    ...
    def resolve(self, secret=None):
        ...
        if not self.is_authorised():
            raise PermissionError("You cannot resolve a PAR that has not been authorised!")
        ...
        result = service.call_function(function="resolve_par", args={"par_uid": self.uid(), "secret": secret})
        ...
        if result["type"] == "DriveMeta":
            ...
            return _Drive.open(_DriveMeta.from_data(result["data"]), creds=creds)
        elif result["type"] == "FileMeta":
            from Acquire.Client import FileMeta as _FileMeta
            from Acquire.Client import File as _File

            return _File.open(_FileMeta.from_data(result["data"]), creds=creds)
