...
class Authorisation:
        ...
        if user is not None:
            ...
            if user.guid() != self.user_guid():
                ...
                raise PermissionError(
                    ...
                    "by multiple identity services: %s versus %s" % (user.guid(), self.user_guid())
                )
    ...
    def _get_user_public_cert(self, scope=None, permissions=None):
        ...
        if not identity_service.can_identify_users():
            raise PermissionError(
                "Cannot verify an Authorisation that does not use a " "valid identity service"
            )
        ...
        if identity_service.uid() != self._identity_uid:
            raise PermissionError(
                ...
                "(%s)" % (self._identity_url, identity_service.uid(), self._identity_uid)
            )

        response = identity_service.get_session_info(
            session_uid=self._session_uid, scope=scope, permissions=permissions
        )
    ...
    def assert_once(self, stale_time=7200, scope=None, permissions=None):
        ...
        if self.is_stale(stale_time):
            if now < self._auth_datetime:
                raise PermissionError(
                    "Cannot assert_once an Authorisation signed " "in the future - please check your clock"
                )
            else:
        ...
        if data is not None:
            raise PermissionError(
                "Cannot auth_once the authorisation as it has been used " "before on this service!"
            )
        ...
        try:
            ...
            raise PermissionError(
                "Cannot auth_once the authorisation as the signature " "is invalid! % s" % str(e)
            )
    ...
    def verify(
        ...
        if self.is_stale(stale_time):
            ...
            if now < self._auth_datetime:
                raise PermissionError(
                    "Cannot verify an Authorisation signed " "in the future - please check your clock"
                )
            else:
        ...
        try:
            ...
            raise PermissionError(
                "Cannot verify the authorisation as the signature " "for resource '%s' is invalid!" % resource
            )
