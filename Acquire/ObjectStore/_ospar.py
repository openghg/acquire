...
__all__ = ["OSPar", "BucketReader", "BucketWriter", "ObjectReader", "ObjectWriter"]
...
class OSPar:
    ...
    def __str__(self):
        ...
        if self._key is None:
            return "OSPar( bucket=True, seconds_remaining=%s )" % (self.seconds_remaining(buffer=0))
        else:
            return "OSPar( key=%s, seconds_remaining=%s )" % (self.key(), self.seconds_remaining(buffer=0))
    ...
    def _get_privkey(self, decrypt_key=None):
        ...
        if decrypt_key is None:
            raise PermissionError("You must supply a private key to decrypt this OSPar")
    ...
    def url(self, decrypt_key=None):
        ...
        if self.seconds_remaining(buffer=30) <= 0:
            ...
            raise PARTimeoutError("The URL behind this OSPar has expired and is no longer valid")
    ...
    def read(self, decrypt_key=None):
        ...
        if not self.is_readable():
            ...
            raise PARPermissionsError("You do not have permission to read from this OSPar: %s" % self)
    ...
    def write(self, decrypt_key=None):
        ...
        if not self.is_writeable():
            ...
            raise PARPermissionsError("You do not have permission to write to this OSPar: %s" % self)
    ...
    def close(self, decrypt_key=None):
        ...
        if service is not None:
            ...
            args = {"par_uid": self._uid, "url_checksum": OSPar.checksum(url)}
...
def _read_remote(url):
    ...
    try:
        ...
        raise PARReadError(
            "Cannot read the remote OSPar URL '%s' because of a possible " "nework issue: %s" % (url, str(e))
        )
...
class BucketReader:
    ...
    def __init__(self, par=None, decrypt_key=None):
        if par:
            if not isinstance(par, OSPar):
                raise TypeError("You can only create a BucketReader from a OSPar")
                ...
                raise PARPermissionsError(
                    "You cannot create a BucketReader from a OSPar without " "read permissions: %s" % par
                )
    ...
    def get_string_object(self, key):
        ...
        try:
            ...
            raise TypeError(
                "The object behind this OSPar cannot be converted to a " "string. Error is: %s" % str(e)
            )
    ...
    def get_all_objects(self, prefix=None):
        ...
        if prefix:
            for name in names:
                objects[name] = self.get_object("%s/%s" % (prefix, name))
        else:
            for name in names:
    ...
    def __init__(self, par=None, decrypt_key=None):
        if par:
            if not isinstance(par, OSPar):
                ...
                raise PARPermissionsError(
                    "You cannot create a BucketWriter from a OSPar without " "write permissions: %s" % par
                )
...
class ObjectReader:
    ...
    def __init__(self, par=None, decrypt_key=None):
        if par:
            if not isinstance(par, OSPar):
                raise TypeError("You can only create an ObjectReader from a OSPar")
            elif par.is_bucket():
                raise ValueError(
                    "You can only create an ObjectReader from a OSPar that " "represents an object: %s" % par
                )
            elif not par.is_readable():
                from Acquire.Client import PARPermissionsError

                raise PARPermissionsError(
                    "You cannot create an ObjectReader from a OSPar without " "read permissions: %s" % par
                )
    ...
    def __init__(self, par=None, decrypt_key=None):
        if par:
            if not isinstance(par, OSPar):
                ...
                raise PARPermissionsError(
                    "You cannot create an ObjectWriter from a OSPar without " "write permissions: %s" % par
                )
