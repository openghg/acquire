...
class GCP_ObjectStore:
    ...
    def create_bucket(bucket, bucket_name):
        ...
        try:
            ...
            bucket_name = _sanitise_bucket_name(bucket_name, bucket["unique_suffix"])
    ...
    def get_bucket(bucket, bucket_name, create_if_needed=True):
        ...
        client = new_bucket["client"]
        sanitised_name = _sanitise_bucket_name(bucket_name, bucket["unique_suffix"])
        ...
        if existing_bucket is None:
            ...
            raise ObjectStoreError(
                "There is not bucket called '%s'. Please check the " "access permissions." % bucket_name
            )
    ...
    def delete_bucket(bucket, force=False):
        ...
        try:
            ...
            raise ObjectStoreError(
                ...
                "access permissions: Error %s" % (bucket["bucket_name"], str(e))
            )
    ...
    def close_par(par=None, par_uid=None, url_checksum=None):
        ...
        if par.driver() != "gcp":
            raise ValueError("Cannot delete a OSPar that was not created " "by the GCP object store")
